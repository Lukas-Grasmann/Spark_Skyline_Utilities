...
# advanced skyliens including HAVING
val df = spark.read.options(Map("inferSchema"->"true","delimiter"->",","header"->"true")).csv("/home/lukas/spark/data/hotels_bahamas_min.csv")
df.registerTempTable("hotels")
val ratings = spark.read.options(Map("inferSchema"->"true","delimiter"->",","header"->"true")).csv("/home/lukas/spark/data/hotels_bahamas_ratings_min.csv")
ratings.registerTempTable("ratings")
val sqlcontext = new org.apache.spark.sql.SQLContext(sc)
