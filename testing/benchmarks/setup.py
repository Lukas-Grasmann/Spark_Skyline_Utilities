# Script for setting up all databases using Spark directly
...
try:
    ...
    sqlContext.sql(f"CREATE TABLE IF NOT EXISTS {database_name}.store_sales_1 AS SELECT * FROM store_sales_incomplete \
                    ...
                    LIMIT 1000000")
    sqlContext.sql(f"CREATE TABLE IF NOT EXISTS {database_name}.store_sales_2 AS SELECT * FROM store_sales_incomplete \
                    WHERE ss_quantity IS NOT NULL AND \
                          ss_wholesale_cost IS NOT NULL AND \
                          ss_list_price IS NOT NULL AND \
                          ss_sales_price IS NOT NULL AND \
                          ...
                          ss_ext_sales_price IS NOT NULL \
                    LIMIT 2000000")
    sqlContext.sql(f"CREATE TABLE IF NOT EXISTS {database_name}.store_sales_5 AS SELECT * FROM store_sales_incomplete \
                    WHERE ss_quantity IS NOT NULL AND \
                          ss_wholesale_cost IS NOT NULL AND \
                          ss_list_price IS NOT NULL AND \
                          ss_sales_price IS NOT NULL AND \
                          ...
                          ss_ext_sales_price IS NOT NULL \
                    LIMIT 5000000")
    sqlContext.sql(f"CREATE TABLE IF NOT EXISTS {database_name}.store_sales_10 AS SELECT * FROM store_sales_incomplete \
                        WHERE ss_quantity IS NOT NULL AND \
                              ss_wholesale_cost IS NOT NULL AND \
                              ss_list_price IS NOT NULL AND \
                              ss_sales_price IS NOT NULL AND \
